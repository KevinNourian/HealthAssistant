...
[LangSmith](https://smith.langchain.com) provides full tracing of every agent run — LLM calls, tool invocations, retrieval results, token counts, and latency — with zero code changes. To enable it, add two variables to your `.env` file:
...
The app will warn at startup if tracing is enabled but the API key is missing. The free tier covers 5,000 traces per month.
