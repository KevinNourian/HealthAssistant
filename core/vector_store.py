...
def normalize_source_path(path: str) -> str:
    ...
    Converts backslashes to forward slashes so that paths
    stored in the Chroma metadata (which may originate on
    Windows) can be reliably compared against paths coming
    from config.json.
    """
...
def _get_ids_by_source(
    vectorstore: Chroma,
) -> Dict[str, List[str]]:
    """Return a mapping of *normalized* source path ->
    list of chunk IDs."""
...
def load_chroma_vectorstore(
    ...
    Args:
        persist_directory: Directory containing the saved
            vector store
        embeddings: Embedding model (default: OpenAIEmbeddings)
...
def get_or_create_vectorstore(
    ...
    """Load existing vector store or create a new one from
    PDFs.

    - Checks if vector store exists
    - Loads from disk if available (and not force_recreate)
    - Otherwise, loads PDFs, chunks them, and creates new
      vector store

    Args:
        pdf_paths: List of PDF file paths to process
        persist_directory: Directory to save/load the store
...
def add_pdf_to_vectorstore(
    ...
    """Embed a single PDF and add its chunks to an existing
    vectorstore."""
    if embeddings is None:
        embeddings = OpenAIEmbeddings()
...
def remove_pdf_from_vectorstore(
    ...
    """Remove all embeddings that originated from
    *pdf_path*."""
...
def sync_vectorstore(
    ...
    - PDFs present in *pdf_paths* but missing from the
      store are embedded.
    - Embeddings whose source PDF is no longer in
      *pdf_paths* are deleted.
    """
    if embeddings is None:
        embeddings = OpenAIEmbeddings()
...
class HybridRetriever(BaseRetriever):
    """Retriever that combines BM25 (keyword) and vector
    (semantic) search.

    Results from both sub-retrievers are merged using
    Reciprocal Rank Fusion (RRF), which produces a single
    ranked list that benefits from exact keyword matches
    *and* semantic similarity.
    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        #   sum( weight / (rank + 60) )  across lists.
        # The constant 60 is the standard RRF smoothing
        # factor.
...
def get_hybrid_retriever(
    ...
    Fetches all stored chunks from Chroma to build an
    in-memory BM25 index, then combines it with the Chroma
    vector retriever using Reciprocal Rank Fusion (RRF)
    with equal weighting.

    Args:
        vectorstore: Chroma vector store instance.
        k: Number of documents each sub-retriever should
            return (default: 3).
